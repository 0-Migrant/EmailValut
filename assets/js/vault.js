...
let state = {
  ...
  deliveryMen: [],  // {id, name, phone}
  orders: [],       // {id, deliveryManId, items:[{itemId,qty,price}], status, note, createdAt}
...
// Order builder state
let orderBuilder = { deliveryManId:'', items:[], note:'' };
...
function load() {
  ...
  if (!state.deliveryMen.length) {
    state.deliveryMen = [
      {id:uid(),name:'Ahmed Hassan',phone:'01001234567'},
      {id:uid(),name:'Mohamed Ali',phone:'01112345678'},
    ];
  }
  pruneHistory();
  applySettingsUI();
...
function dashboard() {
  ...
  <div class="card" style="margin-bottom:20px">
    <div class="card-title">⏳ Pending Orders — Quick View</div>
    ${pending.length ? `<div class="table-wrap"><table>
      <thead><tr><th>#</th><th>Delivery Man</th><th>Items</th><th>Total</th><th>Time</th><th>Actions</th></tr></thead>
...
function newOrder() {
  const pc = document.getElementById('page-content');
  orderBuilder = { deliveryManId:'', items:[], note:'' };
  ...
  <div class="order-layout">
    ...
    <div style="display:flex;flex-direction:column;gap:16px">
      ...
      <div class="card">
        ...
        <div class="field" style="margin-top:10px">
          ...
          <input class="inp" id="ob-note" type="text" placeholder="Special instructions...">
        </div>
      </div>
    ...
    <!-- RIGHT: Item Price List -->
    <div class="item-sidebar">
      <div class="item-sidebar-header">
        <div class="card-title">📋 Price List</div>
        <input class="search-box" id="il-search" type="text" placeholder="Search..." oninput="renderItemSidebar()" style="width:100%;margin-top:6px;height:30px;font-size:12px">
      </div>
      <div class="item-list" id="item-sidebar-list"></div>
    </div>
  </div>`;
...
function renderItemSidebar() {
  ...
  const list = document.getElementById('item-sidebar-list');
  if (!list) return;
  const filtered = state.items.filter(it=>it.name.toLowerCase().includes(q)||it.category?.toLowerCase().includes(q));
  if (!filtered.length) { list.innerHTML='<div class="empty-state" style="padding:20px 0">No items found</div>'; return; }
  list.innerHTML = filtered.map(it => {
    const inOrder = orderBuilder.items.find(x=>x.itemId===it.id);
    return `<div class="item-row${inOrder?' selected':''}" onclick="obAddItemById('${it.id}')">
      <div class="item-name-col">${esc(it.name)}<div style="font-size:10px;color:var(--text-hint)">${esc(it.category||'')}</div></div>
      <input class="item-qty-inp" type="number" min="1" value="${inOrder?inOrder.qty:1}" id="sidqty-${it.id}"
        onclick="event.stopPropagation()"
        onchange="obSetQtyFromSidebar('${it.id}',this.value)">
      <div class="item-price-col">${fmt(it.price)}</div>
    </div>`;
  }).join('');
}

function obAddItemById(itemId) {
  const item = state.items.find(i=>i.id===itemId); if(!item) return;
  const qtyEl = document.getElementById('sidqty-'+itemId);
  const qty = Math.max(1, parseInt(qtyEl?.value||1)||1);
  const existing = orderBuilder.items.find(x=>x.itemId===itemId);
  if (existing) { existing.qty += qty; }
  else { orderBuilder.items.push({itemId, qty, price:item.price}); }
  renderObItems(); renderItemSidebar();
}

function obSetQtyFromSidebar(itemId, val) {
  const qty = Math.max(1, parseInt(val)||1);
  const item = state.items.find(i=>i.id===itemId); if(!item) return;
  const existing = orderBuilder.items.find(x=>x.itemId===itemId);
  if (existing) { existing.qty = qty; } else { orderBuilder.items.push({itemId,qty,price:item.price}); }
  renderObItems();
}
...
function obChangeQty(itemId, val) {
  ...
  if(oi){oi.qty=qty;renderObItems();renderItemSidebar();}
}
...
function submitOrder() {
  ...
  const order = {
    id:uid(), deliveryManId:dmId, items:JSON.parse(JSON.stringify(orderBuilder.items)),
...
function ordersPage() {
  ...
  <div class="flex-row" style="margin-bottom:16px">
    <input class="search-box" id="ord-search" type="text" placeholder="Search delivery man..." oninput="renderOrdersTable()" style="min-width:200px;flex:1">
...
function renderOrdersTable() {
  ...
  let orders=state.orders.filter(o=>{
    const dm=state.deliveryMen.find(d=>d.id===o.deliveryManId);
    const matchQ=!q||(dm&&dm.name.toLowerCase().includes(q));
  ...
  wrap.innerHTML=`<div class="table-wrap"><table>
    <thead><tr><th>#</th><th>Delivery Man</th><th>Items</th><th>Total</th><th>Status</th><th>Date</th><th>Actions</th></tr></thead>
    <tbody>${orders.map(o=>{
      const dm=state.deliveryMen.find(d=>d.id===o.deliveryManId);
      const badgeClass=o.status==='done'?'badge-done':o.status==='cancelled'?'badge-cancelled':'badge-pending';
      return `<tr>
        <td><span class="tag">${o.id.slice(-5)}</span></td>
        <td style="font-weight:500">${esc(dm?dm.name:'Unknown')}</td>
        ...
        <td><div class="action-group">
          ${o.status==='pending'?`<button class="btn btn-success btn-xs" onclick="setOrderStatus('${o.id}','done')">✓ Done</button>
          <button class="btn btn-danger btn-xs" onclick="setOrderStatus('${o.id}','cancelled')">✗ Cancel</button>`:''}
          ${o.status!=='pending'?`<button class="btn btn-ghost btn-xs" onclick="setOrderStatus('${o.id}','pending')">↩ Reset</button>`:''}
          <button class="btn btn-ghost btn-xs" onclick="viewOrder('${o.id}')">View</button>
...
function viewOrder(id) {
  ...
  document.getElementById('order-detail-body').innerHTML=`
    ...
    <div><span class="badge ${badgeClass}">${o.status}</span></div>
    <div class="modal-actions">
      ${o.status==='pending'?`<button class="btn btn-success btn-sm" onclick="setOrderStatus('${o.id}','done');closeOrderModal()">✓ Mark Done</button>
  ...
  <div class="card">
    ...
    <div class="grid-3">
      ${['pending','done','cancelled'].map(s=>{
        ...
        const col=s==='done'?'var(--green)':s==='cancelled'?'var(--red)':'var(--orange)';
  ...
  <div class="grid-2" style="align-items:start">
    <div class="card">
      ...
      <div style="display:flex;flex-direction:column;gap:10px">
        ...
        <div class="field"><label>Phone</label><input class="inp" id="dm-phone" placeholder="010xxxxxxxx" type="tel"></div>
        <div class="flex-row" style="margin-top:4px">
...
function renderDMTable() {
  ...
  el.innerHTML=`<div class="table-wrap"><table>
    <thead><tr><th>Name</th><th>Phone</th><th>Orders</th><th>Revenue</th><th></th></tr></thead>
...
function saveDM() {
  ...
  if(editId){
    ...
    if(dm){dm.name=name;dm.phone=phone;}
    addHist('edit',`Updated delivery man: ${name}`,snap);
  } else {
    state.deliveryMen.push({id:uid(),name,phone});
...
function clearDMForm(){document.getElementById('dm-edit-id').value='';document.getElementById('dm-name').value='';document.getElementById('dm-phone').value='';}
...
function nukeAll(){showConfirm('Delete EVERYTHING','This deletes ALL data permanently. Are you absolutely sure?',()=>{state.items=[];state.deliveryMen=[];state.orders=[];state.credentials=[];state.history=[];persist();navigate('dashboard');});}
...
function exportOrdersPDF(){
  ...
  orders.forEach((o,i)=>{
    ...
    const cardH=28+o.items.length*7+4;
    ...
    const statColor=o.status==='done'?[22,163,74]:o.status==='cancelled'?[220,38,38]:[217,119,6];
